...
def pad_to_wanted_length(input_ids, ideal_length):
    if len(input_ids) >= ideal_length:
            input_ids = input_ids[:ideal_length-2]
    inputs = np.pad(np.array(input_ids, dtype=np.int32), (0, ideal_length - len(input_ids)+1),
                        constant_values=0)
...
def create_dataset(batch_size, data_path, device_num=1, rank=0, drop=True, full_batch=False, data_start_index=0,
    ...
    pad_func = (lambda input_ids: pad_to_wanted_length(input_ids, ideal_length))
    dataset = dataset.map(operations=pad_func, input_columns=[column_name],
    ...
    if eod_reset:
        ...
        time2 = time.time()
        print('mapfun1 Time:', time2-time1, flush=True)
