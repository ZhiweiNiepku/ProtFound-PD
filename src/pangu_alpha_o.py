...
class PanGUAlphaWithLoss(Cell):
    ...
    def __init__(self, config, network, loss):
        ...
        self.create_att_mask = AttentionMask(config.seq_length)
        if config.parallel_config.pipeline_stage > 1:
            self.micro_batch_step = config.parallel_config.micro_batch_num

    def construct(self, input_ids, labels, input_position):
        ...
        # tokens = F.cast(tokens, mstype.int32)
        attention_mask = self.create_att_mask(input_mask)
