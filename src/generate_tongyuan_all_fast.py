...
def topk_fun(logits, topk=5):
    ...
    index = target_column.argsort()[-topk:][::-1]
    return index
...
def generate(model, origin_inputs, config, cur_ss):
    # print("cur_ss", cur_ss)
    """
    Text generation
    ...
    """
    tongyuan_dict = {}
    tongyuan_dict["1"] = [7, 10, 5, 14, 18, 24, 22, 15]
    tongyuan_dict["2"] = [21, 16, 12]
    tongyuan_dict["3"] = [8, 23, 19, 17, 6, 20, 11]
    tongyuan_dict["4"] = [9, 13]

    flag = "0"
    for key in tongyuan_dict.keys():
        tongyuan_values = tongyuan_dict[key]
        if cur_ss in tongyuan_values:
            flag = key
            break
    if flag == "0":
        return cur_ss
    ...
    index = log_probs[0].argsort()[-5:][::-1]
#     print("Top k:", index)
    for ss in index:
        if ss in tongyuan_dict[flag]:
            return ss
